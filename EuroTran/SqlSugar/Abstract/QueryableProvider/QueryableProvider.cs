...
namespace SqlSugar
{
    ...
    public partial class QueryableProvider<T> : QueryableAccessory, ISugarQueryable<T>
    {
        ...
        public virtual ISugarQueryable<T> In<TParamter>(params TParamter[] pkValues)
        {
            ...
            var filed = pks.FirstOrDefault();
            var shortName = QueryBuilder.TableShortName == null ? null : (QueryBuilder.TableShortName + ".");
        ...
        public virtual ISugarQueryable<T> In<FieldType>(string filed, params FieldType[] inValues)
        {
            if (inValues.Length == 1)
            {
                if (inValues.GetType().IsArray)
                {
                    ...
                    var parameterName = this.SqlBuilder.SqlParameterKeyWord + "InPara" + whereIndex;
        ...
        public virtual ISugarQueryable<T> OrderBy(string orderFileds, string orderType = "")
        {
            ...
            QueryBuilder.OrderByValue += " " + orderType;
            return this;
        }
        ...
        public virtual ISugarQueryable<T> MergeTable()
        {
            ...
            var tableName = this.SqlBuilder.GetPackTable(sql, nameof(MergeTable));
        ...
        public virtual List<T> ToPageList(int pageIndex, int pageSize, ref int totalNumber)
        {
            ...
            QueryBuilder.IsDisabledGobalFilter = UtilMethods.GetOldValue(QueryBuilder.IsDisabledGobalFilter, () =>
            {
                ...
                result = count == 0 ? new List<T>() : ToPageList(pageIndex, pageSize);
        ...
        public Task<T> SingleAsync()
        {
            ...
            {
                var asyncQueryable = CopyQueryable();
        ...
        {
            var result = new Task<KeyValuePair<string, int>>(() =>
            {
                var totalNumberAsync = 0;
                var asyncQueryable = CopyQueryable();
        ...
        {
            var result = new Task<KeyValuePair<DataTable, int>>(() =>
            {
                var totalNumberAsync = 0;
                var asyncQueryable = CopyQueryable();
        ...
        {
            var result = new Task<KeyValuePair<List<T>, int>>(() =>
            {
                var totalNumberAsync = 0;
                var asyncQueryable = CopyQueryable();
        ...
        protected List<TResult> GetData<TResult>(KeyValuePair<string, List<SugarParameter>> sqlObj)
        {
            ...
            else result = entityType.IsAnonymousType() || isComplexModel ? this.Context.Utilities.DataReaderToDynamicList<TResult>(dataReader) : this.Bind.DataReaderToList<TResult>(entityType, dataReader);
        ...
        private void SetContextModel<TResult>(List<TResult> result, Type entityType)
        {
            if (result.HasValue())
            {
                if (entityType.GetTypeInfo().BaseType.HasValue() && entityType.GetTypeInfo().BaseType == UtilConstants.ModelType)
                {
                    ...
                    {
                        var contextProperty = item.GetType().GetProperty(nameof(Context));
                        var newClient = this.Context.Utilities.CopyContext();
    ...
    public partial class QueryableProvider<T, T2> : QueryableProvider<T>, ISugarQueryable<T, T2>
    {
        ...
        #region Order
        public new ISugarQueryable<T, T2> OrderBy(string orderFileds, string orderType = null)
        {
            base.OrderBy(orderFileds, orderType);
            return this;
        }
