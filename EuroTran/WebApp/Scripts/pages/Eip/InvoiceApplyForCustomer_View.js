...
var sProgramId = getProgramId(),
    sQueryPrgId = 'InvoiceApplyForCustomer_Qry',
    sEditPrgId = 'InvoiceApplyForCustomer_Upd',
        ...
        var oCurData = { CheckFlows: [], HandleFlows: [], PayeeInfo: [] },
                ...
                return g_api.ConnectLite(sQueryPrgId, ComFn.GetOne,
                    ...
                    function (res) {
                        if (res.RESULT) {
                            ...
                            if ("B".indexOf(oRes.Status) > -1 && SuperAuditor && !oCheck.IsHandler) {
                                if (!oCheck.IsAuditor) {
                                    $('#SuperAuditor').show();
                            ...
                            if (oRes.PaymentType === 'A') {
                                $('.PaymentTime').text(i18next.t("common.CompanyRegulations"));
                            }
                            else {
                                $('.PaymentTime').text(newDate(oRes.PaymentTime, true));
                            }
            ...
            fnSubmitDecision = function (action) {
                ...
                if (action === 'Signed') {
                    ...
                    iCurOder = parseInt(oCurFlow.Order);
                    ...
                    $.each(saNewCheckFlows, function (idx, _flow) {
                        ...
                        if (data.SignedDecision === 'O' && _flow.Order > iCurOder) {
                            _flow.Order = parseInt(_flow.Order) + saNewCheckOrderPush.length;
                    ...
                    let NewCurOrder = saNewCheckFlows[saNewCheckFlows.length - 1].Order;//如果是签辦的話當前順序就是原本流程的最大順序
                    iCurOder = parseInt(NewCurOrder);
                    ...
                    if (data.HandleDecision === 'O') {
                        $.each(saNewCheckOrderPush, function (idx, _push) {
                            _push.Order = idx + parseInt(saNewCheckFlows[saNewCheckFlows.length - 1].Order) + 1;
                        });
                        saAdds = fnCheckFlows({ CheckOrder: saNewCheckOrderPush }, false, false, saUsers);
                    }
                }
                ...
                g_api.ConnectLite(sEditPrgId, 'InvoiceApplyForCustomerAudit', {
                    Guid: oCurData.Guid,
                    Action: action,
                    GoNext: bGoNext ? 'Y' : 'N',
            ...
            fnSubmitDecisionForAll = function () {
                ...
                g_api.ConnectLite(sEditPrgId, 'InvoiceApplyForCustomerAuditAll', {
                    Guid: oCurData.Guid,
             ...
             * 連接到帳單明細
             */
            fnGoToBillInfo = function (billno) {
            ...
            init = function () {
                    ...
                    fnSetArgDrop([
                        {
                            ...
                            CallBack: function (data) {
                                ...
                                $('#RemittanceInformation_TotalCurrencyTW,#RemittanceInformation_TotalCurrency').html(sOptionHtml_Currency)[0].remove(0);
                            }
                        }
                    ])])
                    .done(function () {
                ...
                $("#jsGrid").jsGrid({
                    ...
                    fields: [
                        {
                            name: "FeeItemName", title: 'common.FeeItemName', width: 300, type: "text"
                        },
                        {
                        ...
                        {
                            name: "PrjName", title: 'common.PrjCode', width: 260, type: "text",
                            itemTemplate: function (val, item) {
                                return item.PrjCode + (!val ? '' : '（' + val + '）');
                            }
                        },
                        {
                            name: "Currency", title: 'common.Financial_Currency', width: 80, align: "center", type: "text"
                        },
                        {
                            name: "Amount", title: 'common.Financial_Amount', width: 100, type: "text", align: "right",
                            itemTemplate: function (val, item) {
