    ...
    public class CostAndProfitReportService : ServiceBase
    {
        ...
        public ResponseMessage CVPAnalysisBySaler(RequestMessage i_crm)
        {
            ...
            try
            {
                ...
                {
                    //test 2020/12/4
                    var Test = db.Queryable<OVW_OPM_BillInfo>()
                        .In(t1 => t1.AuditVal, new int[] { 2, 4, 5 })
                        .Select(t1 => new OVW_OPM_BillInfo {
                            Volume = t1.Volume,
                    ...
                    var group = saBills.GroupBy(c => c.ResponsiblePerson).OrderBy(c => c.Key).ToList();
                    ...
                    foreach (IGrouping<string, View_OPM_BillIReport> bills in group)
                    {
                        ...
                        foreach (View_OPM_BillIReport bill in _bills)
                        {
                            ...
                            var ActualCostFeeItems = "";
                            var sActualCost = "";
                            var sTransportationMode = "";

                            CommonRPT.CalcuCostAndProfit(db, ref ActualCostFeeItems, ref sActualCost, ref sTransportationMode, bill.BillNO, bill.ParentId, bill.IsReturn, bill.ReFlow, bill.BillType);
                            //var SharedCost = CommonRPT.GetSharedActualCost(db, ToBillInfo(bill), sActualCost);
                            
                            
                            string PassStatus = CommonRPT.PassStatus;
                            var sAuditVal = bill.AuditVal == null ? "" : bill.AuditVal;
                            var UsedVolume = bill.Volume == null ? "0" : bill.Volume;
                            var shareCost = new ShareCost();
                            double dUsedVolume = 0;
                            double dActualCost = 0;
                            double.TryParse(UsedVolume, out dUsedVolume);
                            double.TryParse(sActualCost, out dActualCost);
                            if (PassStatus.Contains(sAuditVal) && dUsedVolume > 0)
                            {
                                double dVolumes = 0;
                                foreach (var item in Test)
                                {
                                    if (item.OrgID == bill.OrgID && item.ParentId == bill.ParentId && item.IsRetn == bill.IsReturn)
                                    {
                                        double.TryParse(item.Volume, out double Volumes);
                                        dVolumes += Volumes;
                                    }
                                }
                                if (dVolumes == 0)
                            ...
                            var BillReimburseAmount = CommonRPT.GetPrepayForCustomer(saBillPrepayFee, bill.FeeItems, ""); //抓帳單的資料
                            var ActualBillReimburseAmount = CommonRPT.GetPrepayForCustomer(saActualPrepayFee, ActualCostFeeItems);//抓實際成本的資料

                            ProfitInfo profitInfo = new ProfitInfo()
                            {
                                MemberID = bill.ResponsiblePerson,
                                BillUntaxAmt = CommonRPT.Rounding(BillUntaxAmt.Value, RoundingPoint),
                                SharedActualCost = CommonRPT.Rounding(Convert.ToDecimal(shareCost.SharedActualCost), RoundingPoint),
                                //BillReimburseAmount = CommonRPT.Rounding(Convert.ToDecimal(BillReimburseAmount), RoundingPoint),
                                BillReimburseAmount = CommonRPT.Rounding(Convert.ToDecimal(BillReimburseAmount) * decimal.Parse(bill.ExchangeRate == "" ? "1" : bill.ExchangeRate), RoundingPoint),
                                ActualBillReimburseAmount = CommonRPT.Rounding(Convert.ToDecimal(shareCost.Persent * ActualBillReimburseAmount), RoundingPoint)
                            };
                            if (ShowSource)
                            {
                        ...
                        {
                            MemberID = _bills.FirstOrDefault()?.ResponsiblePerson,
        ...
        public ResponseMessage CVPAnalysisByExhibition(RequestMessage i_crm)
        {
            ...
            try
            {
                ...
                {
                    #region 搜尋所有條件
                    var listOvm_Opm_BillInfo = db.Queryable<OVW_OPM_BillInfo>()
                        .In(t1 => t1.AuditVal, new int[] { 2, 4, 5 })
                        .ToList();
                    #endregion
                    ...
                    foreach (IGrouping<string, View_OPM_BillIReport> bills in group)
                    {
                        ...
                        foreach (View_OPM_BillIReport bill in _bills)
                        {
                            ...
                            if (PassStatus.Contains(sAuditVal) && dUsedVolume > 0)
                            {
                                foreach (var item in listOvm_Opm_BillInfo)
                                {
                                    double.TryParse(item.Volume, out double Result);
                                    dVolumes += Result;
                                }
                            }
                            if (dVolumes == 0)
                            ...
                            */
                            ...
                            {
                                BillUntaxAmt = CommonRPT.Rounding(BillUntaxAmt.Value, RoundingPoint),
                                SharedActualCost = CommonRPT.Rounding(SharedCost.SharedActualCost, RoundingPoint),
                                //BillReimburseAmount = CommonRPT.Rounding(BillReimburseAmount, RoundingPoint),
                                BillReimburseAmount = CommonRPT.Rounding(Convert.ToDecimal(BillReimburseAmount) * decimal.Parse(bill.ExchangeRate == "" ? "1" : bill.ExchangeRate), RoundingPoint),
                                ActualBillReimburseAmount = CommonRPT.Rounding(SharedCost.Persent * ActualBillReimburseAmount, RoundingPoint)
                            };
                            if (ShowSource)
                            {
                                profitInfo.BillNO = $"類型:{bill.BillType}-單號:{bill.BillNO}-狀態:{bill.AuditVal}";
                        ...
                        ProfitInfo SubtotalProfitInfo = new ProfitInfo()
                        {
                            MemberID = Exhibition.ExhibitioShotName_TW,
                            BillUntaxAmt = CommonRPT.Rounding(SubProfits.Sum(c => c.BillUntaxAmt), RoundingPoint),
                            SharedActualCost = CommonRPT.Rounding(SubProfits.Sum(c => c.SharedActualCost), RoundingPoint),
                            BillReimburseAmount = CommonRPT.Rounding(SubProfits.Sum(c => c.BillReimburseAmount), RoundingPoint),
                            ActualBillReimburseAmount = CommonRPT.Rounding(SubProfits.Sum(c => c.ActualBillReimburseAmount), RoundingPoint),
                            OrderValue = Exhibition.ExhibitionDateStart == null ? 0 : Exhibition.ExhibitionDateStart.Value.Ticks,
                        };
                        if (ShowSource)
                        {
                            SubtotalProfitInfo.BillNO = string.Join("■", SubProfits.Select(test => test.BillNO));
        ...
        public ResponseMessage CashFlow(RequestMessage i_crm)
        {
            ...
            try
            {
                ...
                {
                    #region 索取所有
                    var test = db.Queryable<OVW_OPM_BillInfo>().In(t1 => t1.AuditVal, new int[] { 2,4,5 }).ToList();
                    #endregion
                    ...
                    var saBills = db.Queryable<OTB_OPM_BillInfo, OTB_OPM_Exhibition, OTB_SYS_Arguments, OTB_CRM_Customers, OTB_CRM_CustomersMST, OTB_SYS_Members>((bill, exp, args, cus, cus_m, mber) => new object[] {
                        ...
                        JoinType.Left, bill.Payer == cus.guid,
                            JoinType.Left, cus.OrgID == cus_m.OrgID && cus.guid == cus_m.customer_guid && cus_m.Effective == "Y",
                    ...
                    foreach (var bill in saBills)
                    {
                        ...
                        CommonRPT.CalcuCostAndProfit(db, ref ActualCostFeeItems, ref sActualCost, ref sTransportationMode, bill.BillNO, bill.ParentId, bill.IsRetn, bill.ReFlow, bill.BillType);
                        var SharedCost = CommonRPT.GetSharedActualCost(db, BillInfo, sActualCost);
                        var BillReimburseAmount = CommonRPT.GetPrepayForCustomer(saBillPrepayFee, bill.FeeItems, ""); //抓帳單的資料
                        var ActualBillReimburseAmount = CommonRPT.GetPrepayForCustomer(saActualPrepayFee, ActualCostFeeItems);//抓實際成本的資料
                        ProfitInfo profitInfo = new ProfitInfo()
                        {
                            BillUntaxAmt = CommonRPT.Rounding(BillUntaxAmt, RoundingPoint),
                            SharedActualCost = CommonRPT.Rounding(Convert.ToDecimal(SharedCost.SharedActualCost), RoundingPoint),
                            //BillReimburseAmount = CommonRPT.Rounding(BillReimburseAmount, RoundingPoint),
                            BillReimburseAmount = CommonRPT.Rounding(Convert.ToDecimal(BillReimburseAmount) * decimal.Parse(bill.ExchangeRate == "" ? "1" : bill.ExchangeRate), RoundingPoint),
                            ActualBillReimburseAmount = CommonRPT.Rounding(SharedCost.Persent * ActualBillReimburseAmount, RoundingPoint)
                        };
            ...
            try
            {
                do
                {
                    ...
                    var Customers = db.Queryable<OTB_CRM_Customers>()
                           ...
                           .ToList();
                    CustomerKey = string.Join(",", Customers.Select(c => c.guid).ToList());
                    #region Exhibition(四展覽)
                    var viewExpIm = db.Queryable<OTB_OPM_ImportExhibition, OTB_CRM_Customers>
                        ((t1, t2) => new object[] { JoinType.Left, t1.Agent == t2.guid })
            ...
            try
            {
                do
                {
                    ...
                    var view = db.Queryable<OVW_OPM_Bills, OTB_OPM_BillInfo, OTB_CRM_Customers>
                        ...
                        new object[] {
                                JoinType.Inner, t1.OrgID == t2.OrgID && t1.BillNO == t2.BillNO,
                                JoinType.Inner, t2.Payer == t3.guid &&  sTransactiontype.Contains(t3.TransactionType)
                              }
                        )
                         .Where((t1, t2) => t1.OrgID == i_crm.ORIGID && CommonRPT.PassStatus.Contains(t2.AuditVal))
        ...
        public ResponseMessage ExistingCustomer(RequestMessage i_crm)
        {
            ...
            try
            {
                do
                {
                    ...
                    //出口
                    var saExportAll = db.Queryable<OTB_OPM_Exhibition, OTB_OPM_ExportExhibition>((t1, t2) => t1.OrgID == i_crm.ORIGID && t1.SN.ToString() == t2.ExhibitionNO && t1.OrgID == t2.OrgID && t1.Effective == "Y" && t2.IsVoid == "N")
                        .WhereIF(!string.IsNullOrWhiteSpace(sResponsibleDeptID), (t1, t2) => SqlFunc.ContainsArray(ChildDeptIDs, t2.DepartmentID))
                        .Select((t1, t2) => new { t1.OrgID, t1.SN, t1.ExhibitionCode, t1.Exhibitioname_TW, t1.Exhibitioname_CN, t2.ExhibitionNO, t2.Exhibitors, t2.Agent }).MergeTable().ToList();

                    //進口
                    var saImportAll = db.Queryable<OTB_OPM_Exhibition, OTB_OPM_ImportExhibition>((t1, t2) => t1.OrgID == i_crm.ORIGID && t1.SN.ToString() == t2.ExhibitionNO && t1.OrgID == t2.OrgID && t1.Effective == "Y" && t2.IsVoid == "N")
                        .WhereIF(!string.IsNullOrWhiteSpace(sResponsibleDeptID), (t1, t2) => SqlFunc.ContainsArray(ChildDeptIDs, t2.DepartmentID))
                        .Select((t1, t2) => new { t1.OrgID, t1.SN, t1.ExhibitionCode, t1.Exhibitioname_TW, t1.Exhibitioname_CN, t2.ExhibitionNO, t2.Suppliers, t2.Supplier, t2.Agent }).MergeTable().ToList();

                    //其他
                    var saOtherAll = db.Queryable<OTB_OPM_Exhibition, OTB_OPM_OtherExhibition>((t1, t2) => t1.OrgID == i_crm.ORIGID && t1.SN.ToString() == t2.ExhibitionNO && t1.OrgID == t2.OrgID && t1.Effective == "Y" && t2.IsVoid == "N")
                        .WhereIF(!string.IsNullOrWhiteSpace(sResponsibleDeptID), (t1, t2) => SqlFunc.ContainsArray(ChildDeptIDs, t2.DepartmentID))
                        .Select((t1, t2) => new { t1.OrgID, t1.SN, t1.ExhibitionCode, t1.Exhibitioname_TW, t1.Exhibitioname_CN, t2.ExhibitionNO, t2.Supplier, t2.Agent }).MergeTable().ToList();

                    //其他駒驛
                    var saOtherTGAll = db.Queryable<OTB_OPM_Exhibition, OTB_OPM_OtherExhibitionTG>((t1, t2) => t1.OrgID == i_crm.ORIGID && t1.SN.ToString() == t2.ExhibitionNO && t1.OrgID == t2.OrgID && t1.Effective == "Y" && t2.IsVoid == "N")
                        .WhereIF(!string.IsNullOrWhiteSpace(sResponsibleDeptID), (t1, t2) => SqlFunc.ContainsArray(ChildDeptIDs, t2.DepartmentID))
                        .Select((t1, t2) => new { t1.OrgID, t1.SN, t1.ExhibitionCode, t1.Exhibitioname_TW, t1.Exhibitioname_CN, t2.ExhibitionNO, t2.Exhibitors, t2.Agent }).MergeTable().ToList();
                    ...
                    foreach (var customer in Customers)
                    {
                        ...
                        //出口
                        var saExport = saExportAll.Where(it => it.Exhibitors.Contains(sId) || it.Agent == sId).ToList();
                        if (saExport.Count > 0)
                        {
                        ...
                        //進口
                        var saImport = saImportAll.Where(it => it.Supplier == sId || it.Agent == sId).ToList();
                        if (saImport.Count > 0)
                        {
                        ...
                        //其他
                        var saOther = saOtherAll.Where(it => it.Supplier == sId || it.Agent == sId).ToList();
                        if (saOther.Count > 0)
                        {
                        ...
                        //其他駒驛
                        var saOtherTG = saOtherTGAll.Where(it => it.Exhibitors.Contains(sId) || it.Agent == sId).ToList();
            ...
            try
            {
                do
                {
                    ...
                    var viewExpIm = db.Queryable<OTB_OPM_ImportExhibition, OTB_CRM_Customers>((t1, t2) => new object[] {
                        JoinType.Left,t1.Agent==t2.guid && t2.Effective == "Y"})
        ...
        {
            var styleForNumber = GetStyle(workbook, 11, false, TextAlignmentType.Right, Color.White, true);
            var styleForText = GetStyle(workbook, 11, false, TextAlignmentType.Left, Color.White, true);
