...
{
    public class ExhibitionExport_UpdService : ServiceBase
    {
        #region 出口報價和預估成本提交審核
        ...
        public ResponseMessage ToAuditForQuote(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sExportBillNO = _fetchString(i_crm, OTB_OPM_ExportExhibition.CN_EXPORTBILLNO);
                        var sSourceID = ((JObject)data[OTB_OPM_ExportExhibition.CN_QUOTE])[EasyNetGlobalConstWord.GUID].ToString();

                        var sdb = new SimpleClient<OTB_OPM_ExportExhibition>(db);
                        var oOpm = sdb.GetById(sExportBillNO);
                        if (oOpm == null)
                        {
                        ...
                        var sTitle = @"「出口」" + oOpm.ExportBillName + @"（" + oOpm.RefNumber + @"）" + @"「報價單/預估成本審核」";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
                        //更新基本資料
                        var oOpmUpd = new OTB_OPM_ExportExhibition
                        {
                            Quote = data[OTB_OPM_ExportExhibition.CN_QUOTE].ToString(),
                            EstimatedCost = data[OTB_OPM_ExportExhibition.CN_ESTIMATEDCOST].ToString(),
                        ...
                        db.Updateable(oOpmUpd)
                            ...
                            .Where(it => it.ExportBillNO == sExportBillNO).ExecuteCommand();
                        //更新代辦
                        ...
                        if (saSupervisor.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, supervisor, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, supervisor, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO);
            ...
            catch (Exception ex)
            {
                ...
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ToAuditForQuote（出口報價和預估成本提交審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ToAuditForQuote Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口報價和預估成本提交審核

        #region 出口報價和預估成本（主管）審核
        ...
        public ResponseMessage AuditForQuote(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = member.MemberName + @"審核了您的「出口」活動名稱：" + oOpm.ExportBillName + @"）的「報價單/預估成本審核」";
                        ...
                        var oOpmUpd = new OTB_OPM_ExportExhibition
                        {
                            Quote = data[OTB_OPM_ExportExhibition.CN_QUOTE].ToString(),
                            EstimatedCost = data[OTB_OPM_ExportExhibition.CN_ESTIMATEDCOST].ToString(),
                            Bills = data[OTB_OPM_ExportExhibition.CN_BILLS].ToString(),
                            ModifyUser = i_crm.USERID,
                            ModifyDate = DateTime.Now
                        };
                        var sAuditVal = ((JObject)data[OTB_OPM_ExportExhibition.CN_QUOTE])[@"AuditVal"].ToString();
                        if (sAuditVal == @"2")
                        {
                            sTitle += @"，審核結果：通過";
                        }
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO, TipsType);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        if (sAuditVal != @"2")
                        {
                            //添加代辦
                            var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO);
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"AuditForQuote（出口報價和預估成本（主管）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.AuditForQuote Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口報價和預估成本（主管）審核
        ...
        public ResponseMessage ToAuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    ...
                    {
                        var sExportBillNO = _fetchString(i_crm, OTB_OPM_ExportExhibition.CN_EXPORTBILLNO);
                        var sExhibitors = _fetchString(i_crm, OTB_OPM_ExportExhibition.CN_EXHIBITORS);
                        var sBills = _fetchString(i_crm, @"Bills");
                        ...
                        var sTitle = @"「出口」" + oOpm.ExportBillName + @"帳單（" + sBillNO + @"）" + @"審核";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
                        ...
                        if (saSupervisor.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, supervisor, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, supervisor, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO);
                                listTask.Add(oTaskAdd);
                            }
                        }
                        else
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ToAuditForBill（出口帳單提交審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ToAuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage AuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = member.MemberName + @"審核了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的「帳單（" + sBillNO + @"）」";
                        ...
                        if (!string.IsNullOrEmpty(oBill[@"Payer"].ToString()))
                        {
                            ...
                            if (oPayer.OrgID != i_crm.ORIGID)
                            {
                                var oMatchPayer = db.Queryable<OTB_CRM_CustomersMST>().Single(it => it.customer_guid == oBill[@"Payer"].ToString() && it.Memo == "CustomerCombine");
                                if (oMatchPayer != null)
                                {
                                    oPayer.CustomerNO = oMatchPayer.CustomerNO;
                                }
                            }
                        }
                        //更新帳單
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, TipsType);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        if (sAuditVal != @"2")
                        {
                            //添加代辦
                            var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO);
                            db.Insertable(oTaskAdd).ExecuteCommand();
                        }
                        ...
                        if (sAuditVal == @"2")
                        {
                            ...
                            string sResponsiblePerson = "";
                            if (oOpm.ResponsiblePerson == "tina.yu")
                            {
                                sResponsiblePerson = oOpm.ResponsiblePerson;
                            }
                            else
                            {
                                sResponsiblePerson = oOpm.ResponsiblePerson.Split('.')[0];
                            }
                            ...
                            oBillsAdd.Url = i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO; //Url
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"AuditForBill（出口帳單（主管）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.AuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口帳單（主管）審核
        ...
        public ResponseMessage CancelAudit(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）取消審核了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的帳單（" + sBillNO + @"）";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        //添加代辦
                        var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO);
                        db.Insertable(oTaskAdd).ExecuteCommand();
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"CancelAudit（（會計）取消審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.CancelAudit Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage WriteOff(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）對您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的帳單（" + sBillNO + @"）進行了銷帳";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"WriteOff（（會計）銷帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.WriteOff Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage CancelWriteOff(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"CancelWriteOff（（會計）取消銷帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.CancelWriteOff Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillPost(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sTitle = @"「出口」（活動名稱：" + oOpm.ExportBillName + @"）的帳單（" + sBillNO + @"）已過帳";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
                        ...
                        if (listAccounts.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, account.MemberID, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=3&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                            }
                        }
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"BillPost（過帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.BillPost Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillCancelPost(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）對您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的帳單（" + sBillNO + @"）取消了過帳";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"BillCancelPost（取消過帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.BillCancelPost Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillVoid(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）作廢了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的帳單（" + sBillNO + @"）";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                        db.Insertable(oTipsAdd).ExecuteCommand();
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"BillVoid（作廢帳單）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.BillVoid Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillDelete(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"管理員（" + member.MemberName + @"）刪除了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的帳單（" + sBillNO + @"）";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"BillDelete（刪除帳單）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.BillDelete Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
            ...
            try
            {
                do
                {
                    ...
                    if (sAction.EndsWith(@"_BatchBill"))
                    {
                        ...
                        {
                            var jaReturn = (JArray)JsonConvert.DeserializeObject(oOpm.ReturnBills);
                            foreach (JObject retn in jaReturn)
                            {
                                if (retn[@"Bills"] != null)
                                {
                                    var jaReturnBills = (JArray)JsonConvert.DeserializeObject(retn[@"Bills"].ToString());
                                    foreach (JObject rebill in jaReturnBills)
                                    {
                                        jaBills.Add(rebill);
                                    }
                                }
                    ...
                    foreach (JObject joBill in jaBills)
                    {
                        string sCurrency = joBill["Currency"] != null ? joBill["Currency"].ToString() : string.Empty;
                        string sPayer = joBill["Payer"] != null ? joBill["Payer"].ToString() : string.Empty;
                        string sBillNO = joBill["BillNO"] != null ? joBill["BillNO"].ToString() : string.Empty;
                        var sAuditVal = joBill["AuditVal"] != null ? joBill["AuditVal"].ToString() : string.Empty;
                        if (@"0,1,2,4,5".IndexOf(sAuditVal) == -1)
                        {
                            continue;
                        }
                        ...
                        var sCurPath = @"";
                        sCurPath = sBase + sOutPut;
                        Common.FnCreateDir(sCurPath);//如果不存在就創建文件夾
                        var sDocxName = @"Invoice-" + sBillNO;
                        //建立臨時文件
                        var sTempFile = Path.GetTempPath() + sDocxName + @".docx";
                        sCurPath += sDocxName + @".docx";
                        ...
                        using (WordprocessingDocument doc = WordprocessingDocument.Open(sTempFile, true))//顯示Word
                        {
                            ...
                            var jaFeeItems = (JArray)JsonConvert.DeserializeObject(joBill[@"FeeItems"].ToString());
                            //查找:獲取第三個表格
                            ...
                            if (jaFeeItems.Count > 0)
                            {
                                ...
                                string sStrAdvance = oBill[@"Advance"] != null ? oBill[@"Advance"].ToString() : string.Empty;
                                //判斷是否有預收款項
                                if (sStrAdvance != "0")
                                {
                                    table.InsertAt<TableRow>(trow_PrePay, iInsertRow + iDefultRow);//插入一行預收
                            ...
                            string sStrIsRetn = joBill[@"IsRetn"] != null ? joBill[@"IsRetn"].ToString() : string.Empty;
                            string sTaxSum = joBill[@"TaxSum"] != null ? joBill[@"TaxSum"].ToString() : string.Empty;
                            string sAmountTaxSum = joBill[@"AmountTaxSum"] != null ? joBill[@"AmountTaxSum"].ToString() : string.Empty;
                            string sAdvance = joBill[@"Advance"] != null ? joBill[@"Advance"].ToString() : string.Empty;
                            string sTotalReceivable = joBill[@"TotalReceivable"] != null ? joBill[@"TotalReceivable"].ToString() : string.Empty;
                            string sWeight = joBill["Weight"] != null ? joBill["Weight"].ToString() : string.Empty;
                            string sVolume = joBill["Volume"] != null ? joBill["Volume"].ToString() : string.Empty;
                            string sNumber = joBill["Number"] != null ? joBill["Number"].ToString() : string.Empty;
                            string sUnit = joBill["Unit"] != null ? joBill["Unit"].ToString() : string.Empty;
                            string sContactorName = joBill["ContactorName"] != null ? joBill["ContactorName"].ToString() : string.Empty;
                            string sTelephone = joBill["Telephone"] != null ? joBill["Telephone"].ToString() : string.Empty;
                            string sMemo = joBill["Memo"] != null ? joBill["Memo"].ToString() : string.Empty;

                            var ReturnType = sStrIsRetn == "Y";
                            var sPARM3 = @"";
                            var sPARM4 = @"";
                            var sPARM7 = oExhibition.Exhibitioname_TW + ExhibitionHelper.GetExhibitioExportComment(false, ReturnType);
                            var sPARM14 = @"";
                            var sPARM16 = @"";
                            double.TryParse(sTaxSum, out var dTaxSum);
                            double.TryParse(sAmountTaxSum, out var dAmountTaxSum);
                            double.TryParse(sAdvance, out var dAdvance);
                            double.TryParse(sTotalReceivable, out var dTotalReceivable);
                            var iPARM18 = dTaxSum;
                            var iPARM19 = dAmountTaxSum;
                            var iPARM20 = dAdvance;
                            var iPARM21 = dTotalReceivable;
                            ...
                            {
                                sPARM7 = oExhibition.Exhibitioname_EN + ExhibitionHelper.GetExhibitioExportComment(true, ReturnType);
                            }
                            if (bTW)
                            {
                                var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                            ...
                            var sDestinationPort = oOpm.Destination ?? @"";
                            string sStrReFlow = joBill["ReFlow"] != null ? joBill["ReFlow"].ToString() : string.Empty;

                            if (sStrIsRetn == @"Y" && joBill[@"SupplierGuid"] != null && !string.IsNullOrEmpty(sStrReFlow))
                            {
                                var sCustomerGuid = joBill[@"CustomerGuid"] != null ? joBill[@"CustomerGuid"].ToString() : string.Empty;
                                var sSupplierGuid = joBill[@"SupplierGuid"] != null ? joBill[@"SupplierGuid"].ToString() : string.Empty;
                                var sReFlow = joBill[@"ReFlow"] != null ? joBill[@"ReFlow"].ToString() : string.Empty;
                            ...
                            var BillCheckDate = string.IsNullOrWhiteSpace(joBill[@"BillCheckDate"]?.ToString()) ? "" : Convert.ToDateTime(joBill[@"BillCheckDate"].ToString()).ToString(@"yyyy/MM/dd");
                            var sBody = body.InnerXml
                                    ...
                                    .Replace(@"[PARM2]", Common.EncodeEscapeChar(ExhibitionHelper.GetBillAddress(oPayer)))
                            ...
                            //若新增PARM變數時候，word也要新增摘要資訊參數(資訊 => 摘要資訊 => 進階摘要資訊 => 新增變數)
                            foreach (JObject jo in JTmpKeys)
                            {
                                string sTemplKey = jo["TemplKey"] != null ? jo["TemplKey"].ToString() : string.Empty;
                                string sTemplKeyValue = jo["TemplKeyValue"] != null ? jo["TemplKeyValue"].ToString() : string.Empty;

                                if (!string.IsNullOrEmpty(sTemplKey))
                                {
                        ...
                        if (File.Exists(sCurPath))
                        {
                            File.Delete(sCurPath);
                        }
                        if (sAction.StartsWith(@"Print"))
                        {
                            sCurPath = sCurPath.Replace(@"docx", @"pdf");
                            var bOk = Common.WordToPDF(sTempFile, sCurPath);
                            if (!bOk)
                            {
                                sCurPath = @"";
                            }
                        ...
                        {
                            File.Copy(sTempFile, sCurPath);
                        }
                        Thread.Sleep(500);
                        File.Delete(sTempFile);   //刪除臨時文件
                        sCurPath = sCurPath.Replace(sBase, @"");
                        saOutPut.Add(sCurPath);
                    }
                    rm = new SuccessResponseMessage(null, i_crm);
                    rm.DATA.Add(BLWording.REL, saOutPut);
                } while (false);
            }
            catch (Exception ex)
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm) + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"PrintBill（出口帳單列印下載）", @"", @"", @"");
                //sMsg += ex.StackTrace; //測試機列印pdf錯誤訊息
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.PrintBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage PrintReceipt(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"PrintReceipt（出口收據列印下載）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.PrintReceipt Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnToAuditForQuote(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sTitle = @"「出口」" + oOpm.ExportBillName + @"（" + oOpm.RefNumber + @"）" + @"「退運" + sIndex + @"報價單/預估成本審核」";
                        ...
                        var oOpmUpd = new OTB_OPM_ExportExhibition
                        {
                            ReturnBills = data[OTB_OPM_ExportExhibition.CN_RETURNBILLS].ToString(),
                            ModifyUser = i_crm.USERID,
                            ModifyDate = DateTime.Now
                        };
                        ...
                        if (saSupervisor.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, supervisor, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, supervisor, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO);
                                listTask.Add(oTaskAdd);
                            }
                        }
                        else
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnToAuditForQuote（出口(退運)報價和預估成本提交審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnToAuditForQuote Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口(退運)報價和預估成本提交審核

        #region 出口(退運)報價和預估成本（主管）審核
        ...
        public ResponseMessage ReturnAuditForQuote(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = member.MemberName + @"審核了您的「出口」活動名稱：" + oOpm.ExportBillName + @"）的「退運" + sIndex + @"報價單/預估成本審核」";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO, TipsType);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        if (sAuditVal != @"2")
                        {
                            //添加代辦
                            var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO);
                            db.Insertable(oTaskAdd).ExecuteCommand();
                        }
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnAuditForQuote（出口(退運)報價和預估成本（主管）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnAuditForQuote Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口(退運)報價和預估成本（主管）審核
        ...
        public ResponseMessage ReturnToAuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    ...
                    {
                        var sExportBillNO = _fetchString(i_crm, OTB_OPM_ExportExhibition.CN_EXPORTBILLNO);
                        var sReturnBills = _fetchString(i_crm, @"ReturnBills");
                        var sExhibitors = _fetchString(i_crm, @"Exhibitors");
                        var sBill = _fetchString(i_crm, @"Bill");
                        var oBill = (JObject)JsonConvert.DeserializeObject(sBill);
                        var sSourceID = oBill[EasyNetGlobalConstWord.GUID].ToString();
                        var sBillNO = oBill[@"BillNO"].ToString();
                        ...
                        var sTitle = @"「出口」" + oOpm.ExportBillName + @"退運帳單（" + sBillNO + @"）" + @"審核";
                        ...
                        var oOpmUpd = new OTB_OPM_ExportExhibition
                        {
                            ReturnBills = sReturnBills,
                            Exhibitors = sExhibitors,
                            ModifyUser = i_crm.USERID,
                            ModifyDate = DateTime.Now
                        };
                        db.Updateable(oOpmUpd).UpdateColumns(it => new { it.ReturnBills, it.Exhibitors, it.ModifyUser, it.ModifyDate })
                            .Where(it => it.ExportBillNO == sExportBillNO).ExecuteCommand();
                        //更新代辦
                        SYS.Task_QryService.TaskStatusUpd(db, i_crm.ORIGID, sSourceID);
                        ...
                        if (saSupervisor.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, supervisor, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, supervisor, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO);
                                listTask.Add(oTaskAdd);
                            }
                        }
                        else
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnToAuditForBill（出口(退運)帳單提交審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnToAuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口（退運）帳單提交審核
        ...
        public ResponseMessage ReturnAuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = member.MemberName + @"審核了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的「退運帳單（" + sBillNO + @"）」";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, TipsType);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        if (sAuditVal != @"2")
                        {
                            //添加代辦
                            var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"ExhibitionExport_Qry", @"?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO);
                            db.Insertable(oTaskAdd).ExecuteCommand();
                        }
                        ...
                        if (sAuditVal == @"2")
                        {
                            ...
                            oBillsAdd.TotalReceivable = oBill[@"TotalReceivable"].ToString(); //總應收
                            oBillsAdd.IsRetn = oBill[@"IsRetn"] == null ? @"N" : oBill[@"IsRetn"].ToString(); //是否為退運
                            oBillsAdd.Url = i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO; //Url
                            db.Insertable(oBillsAdd).ExecuteCommand();
                        }
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnAuditForBill（出口退運帳單（主管）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnAuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 出口退運帳單（主管）審核
        ...
        public ResponseMessage ReturnCancelAudit(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）取消審核了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的退運帳單(" + sBillNO + @")」";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                        db.Insertable(oTipsAdd).ExecuteCommand();
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnCancelAudit（（會計）取消審核（退運））", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnCancelAudit Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnWriteOff(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）對您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的退運帳單（" + sBillNO + @"）進行了銷帳";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnWriteOff（（會計）銷帳（退運））", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnWriteOff Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnCancelWriteOff(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnCancelWriteOff（（會計）取消銷帳（退運））", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnCancelWriteOff Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnBillPost(RequestMessage i_crm)
        {
                ...
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        if (listAccounts.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, account.MemberID, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=5&ExportBillNO=" + oOpm.ExportBillNO + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                            }
                        }
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnBillPost（過帳（退運））", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnBillPost Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnBillCancelPost(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnBillCancelPost（取消過帳（退運））", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnBillCancelPost Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnBillVoid(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）作廢了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的退運帳單（" + sBillNO + @"）";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnBillVoid（作廢（退運）帳單）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnBillVoid Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage ReturnBillDelete(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"管理員（" + member.MemberName + @"）刪除了您的「出口」（活動名稱：" + oOpm.ExportBillName + @"）的退運帳單（" + sBillNO + @"）";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"出口", @"ReturnBillDelete（刪除（退運）帳單）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.ReturnBillDelete Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage UpdateBillInfo(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"", @"UpdateBillInfo（更新帳單明細）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"ExhibitionExport_UpdService.UpdateBillInfo Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        /// <param name="i_crm">帳單資料</param>
        /// <returns></returns>
        public ResponseMessage BatchBillsDownLoad(RequestMessage i_crm)
        {
            ResponseMessage rm = null;
            string sMsg = null;
            var db = SugarBase.GetIntance();
            try
            {
                ...
                {
                    var saOutPut = new List<string>();
                    var bIsRetrun = _fetchBool(i_crm, @"IsRetrun");
                    if (bIsRetrun)
                    {
                    }
                    else
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(ExhibitionExport_UpdService), @"", @"BatchBillsDownLoad（賬單批次下載）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
