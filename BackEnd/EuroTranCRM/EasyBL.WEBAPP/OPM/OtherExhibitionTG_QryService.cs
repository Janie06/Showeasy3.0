...
{
    public class OtherExhibitionTG_QryService : ServiceBase
    {
        #region 其他（分頁查詢）
        ...
        public ResponseMessage QueryPage(RequestMessage i_crm)
        {
            ...
            try
            {
                do
                {
                    ...
                    pml.DataList = db.Queryable<OTB_OPM_OtherExhibitionTG, OTB_SYS_Members, OTB_OPM_Exhibition, OTB_SYS_Arguments, OTB_CRM_Customers>
                        ((t1, t2, t3, t4, t5) =>
                        new object[] {
                                ...
                                JoinType.Left, t1.Agent == t5.guid //t1.OrgID == t5.OrgID
                              }
                        )
                        ...
                        .WhereIF(!string.IsNullOrEmpty(sCustomer), (t1) => t1.Exhibitors.Contains(sCustomer))
                       ...
                       SqlFunc.Subqueryable<OTB_SYS_Members>().Where(p => p.MemberID == t1.ResponsiblePerson && p.OrgID == t1.OrgID).Select(c => c.ImmediateSupervisor) == i_crm.USERID || SqlFunc.ContainsArray(saChildUserIds, t1.CreateUser) || SqlFunc.ContainsArray(saChildUserIds, t1.ResponsiblePerson) || SqlFunc.ContainsArray(saRoles, "Account") || SqlFunc.ContainsArray(saRoles, "CDD") || SqlFunc.ContainsArray(saRoles, "Admin") || SqlFunc.ContainsArray(saRoles, "Manager"))
                        .Select((t1, t2, t3, t4, t5) => new View_OPM_OtherExhibitionTG
                        {
                            OrgID = t1.OrgID,
                            Guid = t1.Guid,
                    ...
                    if (bExcel)
                    {
                        ...
                        var saOtherExhibition = pml.DataList as List<View_OPM_OtherExhibitionTG>;
                        var dt_new = saOtherExhibition.ListToDataTable<View_OPM_OtherExhibitionTG>();
                        switch (sExcelType)
                        {
                            case "OtherBusiness_BasicInformation":
                                sFileName = "其他（駒驛）業務基本資料";
