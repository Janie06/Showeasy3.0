            ...
            try
            {
                do
                {
                    ...
                    foreach (IGrouping<string, View_OPM_BillIReport> bills in group)
                    {
                        ...
                        foreach (View_OPM_BillIReport bill in _bills)
                        {
                            List<CbmVolume> ThisBillCBMUsage = new List<CbmVolume>();

                            if (bill.IsReturn != "Y")
                            {
                                ThisBillCBMUsage = AllCBMUsage.Where(t1 => t1.ParentID == bill.ParentId && t1.IsReturn == bill.IsReturn).ToList();
                            } else
                            {
                                ThisBillCBMUsage = AllCBMUsage.Where(t1 => t1.ParentID == bill.ParentId && t1.IsReturn == bill.IsReturn && t1.ReFlow == bill.ReFlow).ToList();
                            }
                            ...
                            BillReimburseAmount = BillReimburseAmount * decimal.Parse(bill.ExchangeRate == "" ? "1" : bill.ExchangeRate);
