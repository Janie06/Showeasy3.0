...
{
    public class OtherBusiness_UpdService : ServiceBase
    {
        #region 其他報價和預估成本提交審核
        ...
        public ResponseMessage ToAuditForQuote(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sImportBillNO = _fetchString(i_crm, OTB_OPM_OtherExhibition.CN_GUID);
                        var sSourceID = ((JObject)data[OTB_OPM_OtherExhibition.CN_QUOTE])[EasyNetGlobalConstWord.GUID].ToString();
                        ...
                        var sTitle = @"「其他」" + oOpm.ImportBillName + @"「報價單/預估成本審核」";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
                        //更新基本資料
                        var oOpmUpd = new OTB_OPM_OtherExhibition
                        {
                            Quote = data[OTB_OPM_OtherExhibition.CN_QUOTE].ToString(),
                            EstimatedCost = data[OTB_OPM_OtherExhibition.CN_ESTIMATEDCOST].ToString(),
                            ModifyUser = i_crm.USERID,
                            ModifyDate = DateTime.Now
                        };
                        db.Updateable(oOpmUpd).UpdateColumns(it => new { it.Quote, it.EstimatedCost, it.ModifyUser, it.ModifyDate })
                            .Where(it => it.Guid == sImportBillNO).ExecuteCommand();
                        //更新代辦
                        ...
                        if (saSupervisor.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, supervisor, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, supervisor, sTitle, @"OtherBusiness_Qry", @"?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid);
            ...
            catch (Exception ex)
            {
                ...
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"ToAuditForQuote（其他報價和預估成本提交審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
        ...
        #endregion 其他報價和預估成本提交審核

        #region 其他報價和預估成本（主管）審核
        ...
        public ResponseMessage AuditForQuote(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = member.MemberName + @"審核了您的「其他」活動名稱：" + oOpm.ImportBillName + @"）的「報價單/預估成本審核」";

                        var sAuditVal = ((JObject)data[OTB_OPM_OtherExhibition.CN_QUOTE])[@"AuditVal"].ToString();
                        if (sAuditVal == @"2")
                        {
                            sTitle += @"，審核結果：通過";
                        }
                        ...
                        var oOpmUpd = new OTB_OPM_OtherExhibition
                        {
                            Quote = data[OTB_OPM_OtherExhibition.CN_QUOTE].ToString(),
                            EstimatedCost = data[OTB_OPM_OtherExhibition.CN_ESTIMATEDCOST].ToString(),
                            Bills = data[OTB_OPM_OtherExhibition.CN_BILLS].ToString(),
                            ModifyUser = i_crm.USERID,
                            ModifyDate = DateTime.Now
                        };
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid, TipsType);
                        //添加提醒消息
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        if (sAuditVal != @"2")
                        {
                            //添加代辦
                            var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"OtherBusiness_Qry", @"?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid);
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"AuditForQuote（其他報價和預估成本（主管）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.AuditForQuote Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 其他報價和預估成本（主管）審核
        ...
        public ResponseMessage ToAuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sTitle = @"「其他」" + oOpm.ImportBillName + @"帳單（" + sBillNO + @"）" + @"審核";
                        if (i_crm.LANG == @"zh")
                        {
                            sTitle = ChineseStringUtility.ToSimplified(sTitle);
                        }
                        ...
                        if (saSupervisor.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, supervisor, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, supervisor, sTitle, @"OtherBusiness_Qry", @"?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO);
                                listTask.Add(oTaskAdd);
                            }
                        }
                        else
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"ToAuditForBill（其他帳單提交審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.ToAuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage AuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = member.MemberName + @"審核了您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的「帳單（" + sBillNO + @"）」";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO, TipsType);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        if (sAuditVal != @"2")
                        {
                            //添加代辦
                            var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"OtherBusiness_Qry", @"?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO);
                            db.Insertable(oTaskAdd).ExecuteCommand();
                        }
                        ...
                        if (sAuditVal == @"2")
                        {
                            ...
                            oBillsAdd.Url = i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO; //Url
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"AuditForBill（其他帳單（主管）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.AuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 其他帳單（主管）審核
        ...
        public ResponseMessage AccountAuditForBill(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）審核了您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的「帳單(" + sBillNO + @")」";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.CHECK);
                        db.Insertable(oTipsAdd).ExecuteCommand();
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"AccountAuditForBill（其他帳單（會計）審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.AccountAuditForBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }

        #endregion 其他帳單（會計）審核
        ...
        public ResponseMessage CancelAudit(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        sTitle = @"會計（" + member.MemberName + @"）取消審核了您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的帳單（" + sBillNO + @"）";
                        ...
                        //添加提醒消息
                        var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, oOpm.ResponsiblePerson, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                        db.Insertable(oTipsAdd).ExecuteCommand();
                        //添加代辦
                        var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, sSourceID, oOpm.ResponsiblePerson, sTitle, @"OtherBusiness_Qry", @"?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO);
                        db.Insertable(oTaskAdd).ExecuteCommand();
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"CancelAudit（（會計）取消審核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.CancelAudit Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage WriteOff(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        sTitle = @"會計（" + member.MemberName + @"）對您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的帳單（" + sBillNO + @"）進行了銷帳";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"WriteOff（（會計）銷帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.WriteOff Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage CancelWriteOff(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"CancelWriteOff（（會計）取消銷帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.CancelWriteOff Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillPost(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        sTitle = @"「其他」（活動名稱：" + oOpm.ImportBillName + @"）的帳單（" + sBillNO + @"）已過帳";
                        ...
                        if (listAccounts.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, account.MemberID, i_crm.CUSTOMDATA[@"program_id"] + @"|?Action=Upd&GoTab=2&ImportBillNO=" + oOpm.Guid + @"&BillNO=" + sBillNO, WebAppGlobalConstWord.BELL);
                                listTips.Add(oTipsAdd);
                            }
                        }
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"BillPost（過帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.BillPost Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillCancelPost(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        sTitle = @"會計（" + member.MemberName + @"）對您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的帳單（" + sBillNO + @"）取消了過帳";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"BillCancelPost（取消過帳）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.BillCancelPost Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillVoid(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"會計（" + member.MemberName + @"）作廢了您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的帳單（" + sBillNO + @"）";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"BillVoid（作廢帳單）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.BillVoid Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage BillDelete(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var member = db.Queryable<OTB_SYS_Members>().Single(x => x.OrgID == i_crm.ORIGID && x.MemberID == i_crm.USERID);
                        var sTitle = @"管理員（" + member.MemberName + @"）刪除了您的「其他」（活動名稱：" + oOpm.ImportBillName + @"）的帳單（" + sBillNO + @"）";
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"BillDelete（刪除帳單）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.BillDelete Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage PrintBill(RequestMessage i_crm)
        {
                ...
                do
                {
                    ...
                    using (WordprocessingDocument doc = WordprocessingDocument.Open(sTempFile, true))//顯示Word
                    {
                        ...
                        var sBody = body.InnerXml
                                ...
                                .Replace(@"[PARM10]", Common.EncodeEscapeChar(oOpm.ShipAndVoyage ?? @""))
                                .Replace(@"[PARM11]", Common.EncodeEscapeChar(oOpm.BillLadNO ?? @""))
                                .Replace(@"[PARM12]", Common.EncodeEscapeChar(oOpm.ShipmentPort ?? @""))
                                .Replace(@"[PARM13]", Common.EncodeEscapeChar(oOpm.DestinationPort ?? @""))
                                ...
                                .Replace(@"[PARM28]", Common.EncodeEscapeChar(oOpm.BillLadNOSub))
                        ...
                        {
                            if (jo.Property(@"TemplKey") != null)
                            {
                                if (!string.IsNullOrEmpty(jo[@"TemplKey"].ToString()))
                                {
                                    var Value = Common.EncodeEscapeChar(jo[@"TemplKeyValue"].ToString());
                                    var Key = jo[@"TemplKey"].ToString();
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"PrintBill（其他帳單列印下載）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.PrintBill Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage PrintReceipt(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"PrintReceipt（其他收據列印下載）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
                ...
                Logger.Debug(@"OtherBusiness_UpdService.PrintReceipt Debug（Param：" + JsonToString(i_crm) + @"；Response：" + JsonToString(rm) + @"）------------------");
            }
            return rm;
        }
        ...
        public ResponseMessage UpdateBillInfo(RequestMessage i_crm)
        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(OtherBusiness_UpdService), @"其他", @"UpdateBillInfo（更新帳單明細）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
