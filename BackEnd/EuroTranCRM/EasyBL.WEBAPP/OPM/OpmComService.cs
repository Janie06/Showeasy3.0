        ...
        public ResponseMessage InitAppoint(RequestMessage i_crm)
        {
            ...
            try
            {
                do
                {
                    ...
                    var oImportCustomers = db.Queryable<OTB_WSM_PackingOrder, OTB_CRM_Customers>(
                        (t1, t2) => new object[]
                            {
                                JoinType.Inner, t1.CustomerId == t2.guid.ToString()
                            }
                        )
                        .Where((t1, t2) => t1.OrgID == i_crm.ORIGID && t1.AppointNO == sAppointNO && t2.IsAudit == "Y" && t2.Effective == "Y")
                        .Select((t1, t2) => new
                        {
                            t2.guid,
                        })
                    ...
                    oTempl.Supplier = oImportCustomers.guid;

                    //該展覽所有廠商（排除已經產生賬單的資料）
                    var saImportCustomers = db.Queryable<OTB_WSM_PackingOrder, OTB_CRM_Customers>(
                        (t1, t2) => new object[]
                            {
                                JoinType.Inner, t1.CustomerId == t2.guid.ToString()
                            }
                        )
                        .Where((t1, t2) => t1.OrgID == i_crm.ORIGID && t1.ExhibitionNO == oTempl.ExhibitionNO && t2.IsAudit == "Y" && t2.Effective == "Y" && SqlFunc.IsNullOrEmpty(t1.OtherId))
                        .Select((t1, t2) => new
                        {
                            t1.AppointNO,
                            t2.guid,
                    ...
                    var saKeys = saImportCustomers.Select(x => x.guid).ToList();
                    ...
                    var saLast = (from e in saCustomers
                                  join f in saImportCustomers on e.guid equals f.guid
                                  select new
                                  {
                                      f.AppointNO,
                                      e.guid,
        ...
        public ResponseMessage GetBillAssociated(RequestMessage i_crm)
        {
            ...
            try
            {
                do
                {
                    ...
                    var oPackingOrder = db.Queryable<OTB_WSM_PackingOrder, OTB_CRM_Customers>(
                        (t1, t2) => new object[]
                            {
                                JoinType.Inner, t1.CustomerId == t2.guid
                            }
                        )
                        .Where((t1, t2) => t1.OrgID == i_crm.ORIGID && t2.IsAudit == "Y" && t2.Effective == "Y")
                        .WhereIF(!string.IsNullOrEmpty(sAppointNO), (t1, t2) => t1.AppointNO == sAppointNO)
                        .WhereIF(!string.IsNullOrEmpty(sOtherId), (t1, t2) => t1.OtherId == sOtherId)
                        .WhereIF(!string.IsNullOrEmpty(sSupplierID), (t1, t2) => t2.guid == sSupplierID)
                    ...
                    if (oPackingOrder != null)
                    {
                        ...
                        if (jaPackingInfo.Count > 0)
                        {
                            ...
                            var oCustomers = db.Queryable<OTB_WSM_PackingOrder, OTB_CRM_Customers>(
                                (t1, t2) => new object[]
                                    {
                                        JoinType.Inner, t1.CustomerId == t2.guid
                                    }
                                ...
                                .WhereIF(!string.IsNullOrEmpty(sOtherId), (t1, t2) => t1.OtherId == sOtherId)
                                .WhereIF(!string.IsNullOrEmpty(sSupplierID), (t1, t2) => t2.guid == sSupplierID)
                                .Select((t1, t2) => t2).Single();

                            var oContactors = db.Queryable<OTB_CRM_Contactors>()
                                .Where(t1 => t1.CustomerId == oCustomers.guid && t1.ContactorName == oPackingOrder.Contactor).Single();

                            if (!string.IsNullOrEmpty(oCustomers.guid))
                            {
                                sContactorId = oContactors.guid;
                                sContactorName = oContactors.ContactorName;
                            }
                        ...
                        (t1, t2, t3, t4, t5) => new object[]
                                            {
                                               ...
                                               JoinType.Inner, t1.ExhibitionNO == t4.SN.ToString(),
