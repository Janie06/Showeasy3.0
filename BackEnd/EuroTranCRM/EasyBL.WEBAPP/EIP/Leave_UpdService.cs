...
namespace EasyBL.WEBAPP.EIP
{
    public class Leave_UpdService : ServiceBase
    {
        ...
        private Tuple<bool,string,List<OTB_EIP_WenZhong>> GetAnnualRange( SqlSugarClient db,string OrgID, string UserID ,string strStartDate, string strEndDate)
        {
            ...
            {
                var listWZLastYear = AllMatchedWenZhong.Where(it => it.EnableDate <= DateTime.Parse(strStartDate).Date && it.ExpirationDate >= DateTime.Parse(strStartDate).Date && it.RemainHours > 0).ToList();
                var listWZThisYear = AllMatchedWenZhong.Where(it => it.EnableDate <= DateTime.Parse(strEndDate).Date && it.ExpirationDate >= DateTime.Parse(strEndDate).Date && it.RemainHours > 0).ToList();

                var dtExpirationDateLY = listWZLastYear.Min(t => t.ExpirationDate);
                var dtExpirationDateTY = listWZThisYear.Min(t => t.ExpirationDate);

                var saWZLastYear = listWZLastYear.Where(t => t.ExpirationDate == dtExpirationDateLY).First();
                var saWZThisYear = listWZThisYear.Where(t => t.ExpirationDate == dtExpirationDateTY).First();

                //var saWZLastYear = AllMatchedWenZhong.Where(it => it.EnableDate <= DateTime.Parse(strStartDate).Date && it.ExpirationDate >= DateTime.Parse(strStartDate).Date && it.RemainHours > 0).First();
                //var saWZThisYear = AllMatchedWenZhong.Where(it => it.EnableDate <= DateTime.Parse(strEndDate).Date && it.ExpirationDate >= DateTime.Parse(strEndDate).Date && it.RemainHours > 0).First();
                    ...
                    do
                    {
                        ...
                        if (sStatus == @"D-O")
                        {
                            if (oEip.HolidayCategory == @"09")
                            {
                                ...
                                string strGuid = "";
                                foreach (var ALM in AnnualLeaveMemos)
                                {
                                    strGuid = ALM["Guid"].ToString();
                                }

                                var saWenZhong = db.Queryable<OTB_EIP_WenZhong>().Where(it => it.Guid == strGuid).ToList();
