...
namespace EasyBL.WEBAPP.EIP
{
    public class BillChangeApply_UpdService : ServiceBase
    {
        ...
        #region 查詢帳單ALL
        public ResponseMessage GetBillNumber(RequestMessage i_crm)
        {
            ResponseMessage rm = null;
            string sMsg = null;
            ...
            try
            {
                do
                {
                    var saExhibitions = db.Queryable<OVW_OPM_BillInfo> ()
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(BillChangeApply_UpdService), @"展覽管理", @"GetExhibitions（獲取展覽資料）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
        ...
        #region 查詢帳單 BY BillNO
        public ResponseMessage GetBillByGuid(RequestMessage i_crm)
        {
            ResponseMessage rm = null;
            string sMsg = null;
            var sId = _fetchString(i_crm, @"Guid");
            ...
            try
            {
                ...
                {
                    var saExhibitions = db.Queryable<OVW_OPM_BillInfo>()
                        .Where(x => x.OrgID == i_crm.ORIGID && x.BillNO.Contains(sId))
                        .Select(x => new {
                            x.ResponsiblePerson,
                            x.Payer,
                            x.ExhibitionNO,
