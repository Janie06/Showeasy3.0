            ...
            try
            {
                do
                {
                    ...
                    var leaveRequests = db.Queryable<OTB_EIP_LeaveRequest>()
                        ...
                        //.WhereIF(!string.IsNullOrEmpty(sLeaveDateEnd), (t1) => t1.EnableDate <= rLeaveDateEnd.Date && t1.ExpirationDate >= rLeaveDateEnd.Date)
                        .Where(t1 => t1.MemberID == sUserID)
                        .Where(t1 => (t1.EnableDate <= rLeaveDateStart.Date && t1.ExpirationDate >= rLeaveDateStart.Date) || (t1.EnableDate <= rLeaveDateEnd.Date && t1.ExpirationDate >= rLeaveDateEnd.Date))
