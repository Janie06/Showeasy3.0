$(function () {
        ...
        fnSetMenu = function (org) {
            return $.whenArray([
                fnGetWebSiteSetting(function (saRes) {
                    var sHtml = $('<script />', { type: 'text/x-jsrender', html: '<li><a href="{{:Link}}?Id={{:Guid}}">{{:Title}}</a></li>' }).render(saRes);
                    if (saRes.length > 0) {
	...
	if (pname.match('inquiry')){$("li#inquiry").addClass("selected");}else{};
	if (pname.match('exhibitions')){$("li#exhibitions").addClass("selected");}else{};
