    ...
    var sLang = $('[http-equiv="content-language"]').attr('content') || 'zh-TW',
        ...
        */
        fnAppoint = function (confbtn) {
            ...
            return g_api.ConnectLite(Service.apite, 'Appoint', data, function (res) {
                if (res.RESULT) {
                    if (res.DATA.rel) {
                        $.fancybox.close();
                        ...
                        window.location.href = window.location.origin + '/TE/inquiryResult' + (bEn ? '_en' : '') + '.html?AppointNO=' + res.DATA.AppointNO;
                    }
                    else {
