        ...
        fnSetForm_Rule = function () {
            eForm.validate({
                ...
                messages: {
					...
					Unicode: {
                        required: bEn ? 'Please enter the VAT number' : '請輸入統編'
                    },
                    MuseumMumber: {
                    ...
                    AppointTel: {
                        required: bEn ? 'Please enter the company tel number' : '請輸入公司電話'
                    },
                    AppointEmail: {
                        email: bEn ? "The subscriber's Email format is wrong" : '請輸入預約者Email',
                        required: bEn ? 'Please enter the subscriber Email' : '請輸入預約者Email'
                    },
                    Contactor: {
        ...
        init = function () {
            ...
            if (saOrderInfo) {
                ...
                $(".confappoint").click(function () {
					...
					if(formdata.PaymentWay === "1"){
						formdata.PaymentWayDesc = "匯款";
					} else if (formdata.PaymentWay === "2"){
						formdata.PaymentWayDesc = "現場付現";
					}
                    g_db.SetDic('ContactInfo', formdata);
                    window.location.href = window.location.origin + '/TE/inquiryPreview' + (bEn ? '_en' : '') + '.html';
                });
            }
            else {
                window.location.href = window.location.origin + '/TE/inquiry' + (bEn ? '_en' : '') + '.html';
            }
        };
