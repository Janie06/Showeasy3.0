...
function openPageTab(programId, parameters, title) {
    ...
    tabs.find(".fa-times-circle").unbind('click').click(function () {
		var sPrgid = $(this).closest("li")[0].id;
		
		var LiId = $(this).closest('li').attr("aria-controls");    //找到頁面ID(請搜尋<li><a href='#{href}' id='#{id}'>)
		
		if (sPrgid.indexOf('_Upd') > -1 || sPrgid.indexOf('_View') > -1){
			window.editClose = true;
			window.thisId = $(this);
			
			var getUrl = $('#iframe' + LiId).attr('src');       //取得目前的連結(src)
			$("#iframe" + LiId).contents().find('#Toolbar_Leave').click();
		...
		/* if (sPrgid.indexOf('_Upd') > -1 || sPrgid.indexOf('_View') > -1) {//移除當前操作的程式			
