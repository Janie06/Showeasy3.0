...
var sProgramId = getProgramId(),
    ...
    fnPageInit = function () {
        var oCurData = { CheckOrder: [] },
            ...
            setBillNumber = function () {
                return g_api.ConnectLite('BillChangeApply_Upd', 'GetBillNumber', {}, function (res) {
                    if (res.RESULT) {
                        var data = res.DATA.rel;
                        var dataCount = data.length;
                        var optionScript = '<option value="">請選擇...</option>';
                        for (var i = 0; i < dataCount; i++) {
                            optionScript += "<option value='" + data[i]['BillNO'] + "' title='" + data[i]['BillNO'] + "'>";
                            optionScript += data[i]['BillNO'] += "</option>";
                        }
                        $('#BillChanges_BillNO').html(optionScript);
                        select2Init($('#BillChanges_BillNO').parent());
                    }
                })
            },
            /**
            ...
            */
            ...
            init = function () {
                ...
                function fnTrigEpoChange(res) {
                    $("#BillChanges_ExhibitionNO").val(res);
                }
                function fnTrigCusChange(res) {
                    $("#BillChanges_CustomerID").val(res);
                }
                function fnTrigRespersonChange(res) {
                    $("#BillChanges_ResponsiblePerson").val(res);
                }
                $("#BillChanges_BillNO").on('change', function () {
                    var Guid = $(this).val();
;                    return g_api.ConnectLite('BillChangeApply_Upd', 'GetBillByGuid', {
                            Guid: Guid 
                        },
                        function (res) {
                            if (res.RESULT) {
                                var data = res.DATA.rel;
                                $("#BillChanges_ExhibitionNO").val(data[0]['ExhibitionCode']).change();
                                $("#BillChanges_CustomerID").val(data[0]['Payer']).change();
                                $("#BillChanges_ResponsiblePerson").val(data[0]['ResponsiblePerson']).change();
                            }
                    })
                });
