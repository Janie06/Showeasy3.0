    ...
    fnPageInit = function () {
        ...
        var oValidator = null,
            ...
            setTime = function (that) {
                var sDate = newDate(null, true);
                if (that.checked) {
                    if (that.id === 'Import_GoodsArrival_Checked') {
                        sDate = $('#ArrivalTime').val() == '' ? sDate : $('#ArrivalTime').val();
                    }
                    else if (that.id === 'Import_Sign_Checked') {
                        sDate = $('#ApproachTime').val() == '' ? sDate : $('#ApproachTime').val();
                    }
                    $(that).parent().next().find('.date-picker').val(sDate);
                }
                else {
                    $(that).parent().next().find('.date-picker').val('');
                }
            },
            /**
             * 獲取簽名檔
                ...
                switch (data.KeyName) {
                    ...
                    case 'EstimatedCost':
                        if (oTab[0].id === 'tab3') {
                            ...
                            $('#tab9 .return_estimatedcostsum').val(fMoney(Current_Return_Estimatedcostsum + iTaxSubtotal, 2, data.Currency));
                        }
                        break;
                    case 'Bill':
                        ...
                        if (oTab[0].id === 'tab3') {
                            ...
                            oFinancial.find('.estimatedcostsum').val(fMoney(parentdata.EstimatedCost.AmountTaxSum, 2, data.Currency));
                            if (data.AuditVal !== '6') {
                                ...
                                let Current_Return_Amountsum = TabTipTaxtotal + TabTipUntaxtotal;
                                $('#tab9 .return_amountsum').val(fMoney(LastRow_Return_Amountsum + Current_Return_Amountsum, 2, FeeItemCurrency));
                ...
                var oConfirm = $('<input />', {
                        class: 'jsgrid-button jsgrid-update-button', type: 'button', title: i18next.t('common.Confirm'), click: function () {// ╠common.Confirm⇒確認╣
                            ...
                            if (data.FinancialNumber.indexOf('.00') > 0)
                                data.FinancialNumber = data.FinancialNumber.replace('.000', '');
                            switch (sId) {
                                case 'quote-box':
                                    if (feeinfo) {
                                        $.each(parentdata.Quote.FeeItems, function (idx, item) {
                    ...
                    $.each(oCurData.Bills, function (idx, bill) {
                        ...
                        oBillBox.find('[data-id="Currency"]').html(sAccountingCurrencyOptionsHtml).on('change', function () {
                            ...
                            if (MainCurrency) {
                                ...
                                fnBindFeeItem($('[data-id=bill_fees_' + bill.BillNO + ']'), oGlobalItem.parentdata, bill);
                            }
                            else {
                                TitleAttr = 'common.ForeignCurrencyNontaxedAmountV2';
