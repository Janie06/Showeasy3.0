...
var sProgramId = getProgramId(),
    ...
    fnPageInit = function () {
		...
		var setListSourceDrop = function () {
			return g_api.ConnectLite('Customers_Upd', 'GetListSource', {}, function (res) {
				if (res.RESULT) {
					var saList = res.DATA.rel;
					$('#ListSource').html(createOptions(saList, 'guid', 'CustomerShotCName')+
						'<option value="SelfCome" title="自來">自來</option><option value="ImportFromDB" title="資料庫匯入">資料庫匯入</option>'
					).select2();
				}
			});
        };
        var oForm = $('#form_main'),
            ...
            init = function () {
                ...
                var saFields = [
					...
					{
						name: "ResponsiblePersonName", title: '負責業務', type: "text", width: 70
					},
					{
					...
					{
                        name: "IsDeal", title: '是否成交', type: "text", width: 50, align: "center", itemTemplate: function (val, item) {
                            return val === 'Y' ? $('<span />', { text: "已成交" }).css('color', 'blue') : "未成交";// ╠common.Effective⇒有效╣ ╠common.Invalid⇒無效╣
                        }
                    },
                    {
