    ...
    CanDo.fn = CanDo.prototype = {
        ...
        _pageLeave: function () {
            var cando = this,
				intLeave = 0,
                toLeave = function () {
					parent.top.msgs.server.removeEditPrg(cando.ProgramId).done(function() {
						if (intLeave === 1) {
							var pPrevLi = parent.top.thisId.closest("li").prev().find('a'),
								pLiId = parent.top.thisId.closest("li").remove().attr("aria-controls");
							pPrevLi.click();
						} else {
			...
			if (typeof parent.top.editClose != "undefined") {
				if(parent.top.editClose){
					parent.top.editClose = false;
					intLeave = 1;
				}
			}
