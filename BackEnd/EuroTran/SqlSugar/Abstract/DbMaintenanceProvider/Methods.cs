...
namespace SqlSugar
{
    public abstract partial class DbMaintenanceProvider : IDbMaintenance
    {
        ...
        {
            var cacheKey = "DbMaintenanceProvider.GetViewInfoList";
        ...
        {
            var cacheKey = "DbMaintenanceProvider.GetTableInfoList";
            cacheKey = GetCacheKey(cacheKey);
        ...
        public virtual List<DbColumnInfo> GetColumnInfosByTableName(string tableName)
        {
            ...
            var cacheKey = "DbMaintenanceProvider.GetColumnInfosByTableName." + this.SqlBuilder.GetNoTranslationColumnName(tableName).ToLower();
            cacheKey = GetCacheKey(cacheKey);
        ...
        {
            var cacheKey = "DbMaintenanceProvider.GetIsIdentities" + this.SqlBuilder.GetNoTranslationColumnName(tableName).ToLower();
            cacheKey = GetCacheKey(cacheKey);
        ...
        {
            var cacheKey = "DbMaintenanceProvider.GetPrimaries" + this.SqlBuilder.GetNoTranslationColumnName(tableName).ToLower();
            cacheKey = GetCacheKey(cacheKey);
            return this.Context.Utilities.GetReflectionInoCacheInstance().GetOrCreate(cacheKey,() =>
            {
        ...
        public virtual bool IsAnySystemTablePermissions()
        {
            ...
            var sql = this.CheckSystemTablePermissionsSql;
            try
            {
        ...
        public virtual bool AddPrimaryKey(string tableName, string columnName)
        {
            ...
            var sql = string.Format(this.AddPrimaryKeySql, tableName, string.Format("PK_{0}_{1}", this.SqlBuilder.GetNoTranslationColumnName(tableName), this.SqlBuilder.GetNoTranslationColumnName(columnName)), columnName);
            this.Context.Ado.ExecuteCommand(sql);
            return true;
        }
        ...
        {
            tableName = this.SqlBuilder.GetTranslationTableName(tableName);
            var sql = GetAddColumnSql(tableName, columnInfo);
            this.Context.Ado.ExecuteCommand(sql);
            return true;
        }
        ...
        {
            tableName = this.SqlBuilder.GetTranslationTableName(tableName);
            var sql = GetUpdateColumnSql(tableName, column);
            this.Context.Ado.ExecuteCommand(sql);
            return true;
        }
        ...
        public virtual bool BackupTable(string oldTableName, string newTableName, int maxBackupDataRows = int.MaxValue)
        {
            ...
            var sql = string.Format(this.BackupTableSql, newTableName, oldTableName, maxBackupDataRows);
            this.Context.Ado.ExecuteCommand(sql);
            return true;
        }
        public virtual bool RenameColumn(string tableName, string oldColumnName, string newColumnName)
        {
            ...
            var sql = string.Format(this.RenameColumnSql, tableName, oldColumnName, newColumnName);
            this.Context.Ado.ExecuteCommand(sql);
            return true;
        }
        #endregion
        ...
        protected virtual string GetCreateTableSql(string tableName, List<DbColumnInfo> columns)
        {
            ...
            {
                var columnName = this.SqlBuilder.GetTranslationTableName(item.DbColumnName);
                var dataType = item.DataType;
                var dataSize = GetSize(item);
                ...
                var addItem = string.Format(this.CreateTableColumn, columnName, dataType, dataSize, nullType, primaryKey, identity);
        ...
        protected virtual string GetAddColumnSql(string tableName, DbColumnInfo columnInfo)
        {
            ...
            var dataSize = GetSize(columnInfo);
            var nullType = columnInfo.IsNullable ? this.CreateTableNull : CreateTableNotNull;
            string primaryKey = null;
            string identity = null;
            var result = string.Format(this.AddColumnToTableSql, tableName, columnName, dataType, dataSize, nullType, primaryKey, identity);
            return result;
        }
        protected virtual string GetUpdateColumnSql(string tableName, DbColumnInfo columnInfo)
        {
            var columnName = this.SqlBuilder.GetTranslationTableName(columnInfo.DbColumnName);
            tableName = this.SqlBuilder.GetTranslationTableName(tableName);
            var dataType = columnInfo.DataType;
            var dataSize = GetSize(columnInfo);
            var nullType = columnInfo.IsNullable ? this.CreateTableNull : CreateTableNotNull;
            string primaryKey = null;
            string identity = null;
            var result = string.Format(this.AlterColumnToTableSql, tableName, columnName, dataType, dataSize, nullType, primaryKey, identity);
            return result;
        }
