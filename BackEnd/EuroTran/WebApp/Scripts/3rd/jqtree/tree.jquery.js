...
//綁定事件
function bindTreeEvent(callback, handle) {
    var $tree = handle || $('#tree_menu');
    $tree.unbind('tree.click').on('tree.click', function (event) {
        var node = event.node;
        if ($('.jqtree-selected').length > 0) {
        ...
        if (node.type === 'c') {
            ...
            try {
                closeNextnode($tree, node);
            }
            catch (e) {
            }
            closePrenode($tree, node);
        }
    }
);
}
//關閉下一個模組
function closeNextnode($tree, node) {
    var nodeNext = node.getNextSibling();
    if (nodeNext) {
        $tree.tree('closeNode', nodeNext);
        closeNextnode($tree, nodeNext)
    }
}
//關閉上一個模組
function closePrenode($tree, node) {
    var nodePre = node.getPreviousSibling();
    if (nodePre) {
        $tree.tree('closeNode', nodePre);
        closePrenode($tree, nodePre);
    }
}