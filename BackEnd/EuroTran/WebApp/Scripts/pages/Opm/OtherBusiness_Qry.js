                    ...
                    .done(function () {
                        ...
                        $("#jsGrid").jsGrid({
                            ...
                            rowClick: function (args) {
                                if ((args.item.IsVoid === 'Y' && parent.UserInfo.IsManager) || args.item.IsVoid === 'N') {
                                    if (navigator.userAgent.match(/mobile/i)) {
                                        goToEdit(sEditPrgId, '?Action=Upd&ImportBillNO=' + args.item.Guid);
                                    }
                                }
                                else {
                            ...
                            rowDoubleClick: function (args) {
                                if ((args.item.IsVoid === 'Y' && parent.UserInfo.IsManager) || args.item.IsVoid === 'N') {
                                    parent.openPageTab(sEditPrgId, '?Action=Upd&ImportBillNO=' + args.item.Guid);
                                }
                                else {
                                    showMsg(i18next.t("message.NoPermission")); // ╠message.NoPermission⇒沒有權限╣
                                }
                            ...
                            fields: [
                                ...
                                {
                                    name: "CustomerNM", title: 'common.Customer', type: "text", width: 150
                                },
                                    ...
                                    type: "control", title: 'common.Bill_Status', itemTemplate: function (val, item) {// ╠common.Bill_Status⇒帳單狀態╣
                                        ...
                                        if (saBills.length > 0) {
                                            ...
                                            $.each(saBills, function (idx, bill) {
                                                ...
                                                sTipsHtml += "<li><a class='gopagetab' onclick='parent.openPageTab(\"" + sEditPrgId + "\",\"?Action=Upd&GoTab=2&ImportBillNO=" + item.Guid + "&BillNO=" + bill.BillNO + "\")' ><div>" + bill.BillNO + "</div><div>" + sStatusName + "</div></a></li>";
                                            });
                                            sTipsHtml += '</ul>';
