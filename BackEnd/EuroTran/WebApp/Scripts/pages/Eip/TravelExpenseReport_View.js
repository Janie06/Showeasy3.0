...
var sProgramId = getProgramId(),
    sQueryPrgId = 'TravelExpenseReport_Qry',
    sEditPrgId = 'TravelExpenseReport_Upd',
    sAction = getUrlParam('Action') || 'Add',
    sDataId = getUrlParam('Guid'),
    sCheckId = sDataId,
    fnPageInit = function () {
        var oCurData = { CheckFlows: [], HandleFlows: [], TravelFeeItems: [] },
            oForm = $('#form_main'),
            saCheckOrder_Push = [],
            saUsers = [],
            ...
            /**
             * 註冊加簽事件
             * @param {Number} _grididx當前Gird 序號
             */
                ...
                return g_api.ConnectLite(sQueryPrgId, ComFn.GetOne,
                    ...
                    function (res) {
                        if (res.RESULT) {
                            ...
                            fnCheckReEdit(oRes.Status, oRes.Applicant);
                            
                            if (oRes.Applicant !== parent.UserID) {
                                $('#Toolbar_Copy').remove();
                            }
                            ...
                            $('#status').text(sStatus);
                            $('#Applicant').text(oCurData.DeptName + ' ' + oCurData.ApplicantName);
                            ...
                            $('.ExhibitionName').text(oCurData.ExhibitionNO + " -" + oCurData.ExhibitionName);
                            $('.TravelPlace').text(oCurData.TravelPlace);
                            $('.TravelDate').text(newDate(oCurData.TravelDateStart, 'date', true) + ' ~ ' + newDate(oCurData.TravelDateEnd, 'date', true));
                            ...
                            moneyInput($('[data-type="int"]'), 0, true);
            ...
            fnCopy = function () {
                ...
                data.TravelFeeItems = JSON.stringify(data.TravelFeeItems);
            ...
            fnSubmitDecision = function (action) {
                ...
                g_api.ConnectLite(sEditPrgId, 'TravelExpenseReportAudit', {
                    Guid: oCurData.Guid,
                    Action: action,
                    GoNext: bGoNext ? 'Y' : 'N',
                    ...
                    if (res.RESULT) {
                        showMsgAndGo(i18next.t('message.' + action + '_Success'), sProgramId, '?Action=Upd&Guid=' + oCurData.Guid);// ╠message.Signed_Failed⇒簽核成功╣ ╠message.Handle_Failed⇒簽辦成功╣
            ...
            init = function () {
                ...
                $("#jsGrid").jsGrid({
                    ...
                    fields: [
                        {
                            name: "Date", title: 'Date', width: 100, type: "text"
                        },
                        {
                            name: "Particulars", title: 'PARTICULARS', width: 200, type: "text"
                        },
                        {
                            name: "Currency", title: 'common.Financial_Currency', width: 80, type: "text", align: "center"
                        },
                        {
                            name: "Amount1", title: 'common.TravelExpenseFee1', width: 100, type: "text", align: "right",
                            itemTemplate: function (val, item) {
                                return fMoney(item.Amount1, 2, item.Currency);
                            }
                        },
                        {
                            name: "Amount2", title: 'common.TravelExpenseFee2', width: 100, type: "text", align: "right",
                            itemTemplate: function (val, item) {
                                return fMoney(item.Amount2, 2, item.Currency);
                            }
                        },
                        {
                            name: "Amount3", title: 'common.TravelExpenseFee3', width: 100, type: "text", align: "right",
                            itemTemplate: function (val, item) {
                                return fMoney(item.Amount3, 2, item.Currency);
                            }
                        },
                        {
                            name: "ExchangeRate", title: 'common.ExchangeRate', width: 80, type: "text", align: "center"
                        },
                        {
                            name: "Total", title: 'TWD', width: 100, type: "text", align: "right",
                            itemTemplate: function (val, item) {
...
require(['base', 'jsgrid', 'formatnumber', 'jqprint', 'filer', 'common_eip', 'util'], fnPageInit);