...
var sProgramId = getProgramId(),
    sQueryPrgId = 'Leave_Qry',
    sEditPrgId = 'Leave_Upd',
    ...
    fnPageInit = function () {
        var oCurData = { CheckFlows: [], HandleFlows: [] },
                ...
                return g_api.ConnectLite(sQueryPrgId, ComFn.GetOne,
                    ...
                    function (res) {
                        if (res.RESULT) {
                            ...
                            $('#Recipient').text($('#Agent_Person option:selected').text() + '->' + saFlowsText.join('->'));
            ...
            fnCopy = function () {
                ...
                data.CheckFlows = fnCheckFlows(data, true, true, saUsers);
                ...
                delete data.AnnualLeaveMemo;
                delete data.RowIndex;
                delete data.AskTheDummyName;
                delete data.Agent_PersonName;
                delete data.Handle_PersonName;
                delete data.HolidayCategoryName;
                delete data.DeptName;
                delete data.WenZhongAcount;
             ...
             */
            fnReEdit = function () {
                g_api.ConnectLite(sEditPrgId, 'SetReEdit', {
                    guid: oCurData.Guid
                }, function (res) {
                    if (res.RESULT) {
                        showMsgAndGo(i18next.t('message.ReEdit_Success'), sProgramId, '?Action=Upd&Guid=' + oCurData.Guid);// ╠message.ReEdit_Success⇒抽單成功╣
                    }
                    else {
            ...
            fnSubmitDecision = function (action) {
                ...
                g_api.ConnectLite(sEditPrgId, 'LeaveAudit', {
                    Guid: oCurData.Guid,
                    Action: action,
                    GoNext: bGoNext ? 'Y' : 'N',
                    ...
                    if (res.RESULT) {
                        if (res.DATA.Status === 'E') {
                            //如果審核狀態是待經辦（所有人都審核完）的話就同步更新至行事曆和outlook中
                            fnSynChronousCalendar();
                        }
             ...
             * 同步新更新行事曆和outlook
             */
            fnSynChronousCalendar = function () {
                var data = {};
                data.CalType = oCurData.HolidayCategory === '12' ? '08' : '07';//07:請假||08:出差
                data.Title = oCurData.AskTheDummyName + '（' + oCurData.DeptName + '） ' + '「' + oCurData.HolidayCategoryName + '」' + oCurData.KeyNote;
                data.Description = i18next.t("common.LeaveReason") + '：' + oCurData.LeaveReason;
                data.StartDate = oCurData.StartDate;
                data.EndDate = oCurData.EndDate;
                data.Color = parent.UserInfo.CalColor;
                data.RelationId = oCurData.Guid;
                data.GroupMembers = '';
                data.Importment = oCurData.Important === '1' ? 'M' : 'H';
                data.OpenMent = 'C';
                data.AllDay = '0';
                data.Memo = 'leave';
                data.UserID = oCurData.AskTheDummy;
                        ...
                        if (res.RESULT) {
                            ...
                            if (parent.Outklook) {
                                outlookAPI(outlook.Calendar_Add, {
                                    ...
                                    AskTheDummy: oCurData.AskTheDummyName,
                                    HolidayCategoryName: oCurData.HolidayCategoryName
                                });
                            }
                        }
                    });
