    ...
    FormatNumber.prototype = {
        init: function (settings) {
            ...
            $(settings.trigger, parent).each(function (i, v) {
                ...
                if ($(v)[0].tagName == 'INPUT') {
                    ...
                    if ($(v).attr('data-name')) {
                        ...
                        $(v).parent().append('<input data-rule="number" id="' + id + '" type="hidden" />');
            ...
            $(settings.trigger, parent).on('keyup keydown', function (e) {
                var that = this, _e = e;
                setTimeout(function () {
                    if (_e.keyCode === 9) { return false; }//當是tab鍵時不做動作
                    checkNumber.call(that, _e);
                }, 10);//全選時直接key入取代
            });
        ...
        doFormat: function (s) {
            ...
            if (typeof s === 'string') {
                s = s.replace(/^(\d+)((\.\d*)?)$/, function (v1, v2, v3) {
                    var sInteger = v2.replace(/\d{1,3}(?=(\d{3})+$)/g, '$&,'),
                        sDecimal = (v3 || '.00000000000').slice(0, _this.settings.decimal + 1);
                    return _this.settings.decimal === 0 ? sInteger : sInteger + sDecimal;
                });
            }
