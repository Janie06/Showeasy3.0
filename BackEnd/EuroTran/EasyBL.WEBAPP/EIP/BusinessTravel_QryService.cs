...
{
    public class BusinessTravel_QryService : ServiceBase
    {
        #region 出差申請單分頁查詢
        ...
        public ResponseMessage QueryPage(RequestMessage i_crm)
        {
            ...
            try
            {
                do
                {
                    ...
                    pml.DataList = db.Queryable<OTB_EIP_BusinessTravel, OTB_SYS_Members, OTB_SYS_Members, OTB_SYS_Members, OTB_SYS_Arguments>
                        ((t1, t2, t3, t4, t5) =>
                        new object[] {
                                ...
                                JoinType.Inner, t1.OrgID == t5.OrgID && t1.HolidayCategory == t5.ArgumentID && t5.ArgumentClassID=="BTrlType"
                              }
                        )
                        ...
                        .Select((t1, t2, t3, t4, t5) => new View_EIP_BusinessTravel
                        {
                    ...
                    if (bExcel)
                    {
                        ...
                        var saBusinessTravel = pml.DataList as List<View_EIP_BusinessTravel>;
                        foreach (var item in saBusinessTravel)
                        {
            ...
            catch (Exception ex)
            {
                ...
                LogAndSendEmail(sMsg + "Params：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, "EasyBL.WEBAPP.EIP.BusinessTravel_QryService", "", "QueryPage（出差申請單分頁查詢）", "", "", "");
            }
            finally
            {
                if (null != sMsg)
        ...
        public ResponseMessage QueryOne(RequestMessage i_crm)
        {
            ...
            try
            {
                do
                {
                    ...
                    var oEntity = db.Queryable<OTB_EIP_BusinessTravel, OTB_SYS_Members, OTB_SYS_Departments, OTB_SYS_Members, OTB_SYS_Members, OTB_SYS_Arguments>
                        ((t1, t2, t3, t4, t5, t6) =>
                        new object[] {
                                JoinType.Inner, t1.OrgID == t2.OrgID && t1.AskTheDummy == t2.MemberID,
                                ...
                                JoinType.Inner, t1.OrgID == t6.OrgID && t1.HolidayCategory == t6.ArgumentID && t6.ArgumentClassID=="BTrlType"
                              }
                        )
                        .Where((t1, t2, t3, t4, t5, t6) => t1.OrgID == i_crm.ORIGID && t1.Guid == sId)
                        .Select((t1, t2, t3, t4, t5, t6) => new View_EIP_BusinessTravel
                        {
                            Guid = SqlFunc.GetSelfAndAutoFill(t1.Guid),
                            WenZhongAcount = t2.WenZhongAcount,
                            AskTheDummyName = t2.MemberName,
                    ...
                    {
                        var oRelation = db.Queryable<OTB_EIP_BusinessTravel>().Single(x => x.OrgID == i_crm.ORIGID && x.Guid == oEntity.RelationId);
                        if (oRelation != null)
                        {
            ...
            {
                sMsg = Util.GetLastExceptionMsg(ex);
                LogAndSendEmail(sMsg + "Params：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, "EasyBL.WEBAPP.EIP.BusinessTravel_QryService", "", "QueryOne（出差申請單單筆查詢）", "", "", "");
            }
            finally
            {
                if (null != sMsg)
