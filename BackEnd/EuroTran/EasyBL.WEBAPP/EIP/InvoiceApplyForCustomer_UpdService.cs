...
{
    public class InvoiceApplyForCustomer_UpdService : ServiceBase
    {
        #region 請款單（廠商）提交簽核
        ...
        /// 請款單（廠商）提交簽核
        /// </summary>
        /// <param name="i_crm">todo: describe i_crm parameter on InvoiceApplyForCustomerToAudit</param>
        /// <returns></returns>
        public ResponseMessage InvoiceApplyForCustomerToAudit(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sTitle = oApplicant.MemberName + @"的請款單（廠商）申請「" + oEip.KeyNote + @"」簽呈編號：" + oEip.SignedNumber;
                        if (i_crm.LANG == @"zh")
                        {
                        ...
                        if (saSignedId.Count > 0)
                        {
                            ...
                            {
                                //添加提醒消息
                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle, signedId, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, WebAppGlobalConstWord.BELL);
                                db.Insertable(oTipsAdd).ExecuteCommand();
                                //添加代辦
                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, oEip.Guid, signedId, sTitle, @"InvoiceApplyForCustomer_View", @"?Action=Upd&Guid=" + oEip.Guid, @"B");
            ...
            catch (Exception ex)
            {
                ...
                LogAndSendEmail(sMsg + @"Param：" + JsonToString(i_crm), ex, i_crm.ORIGID, i_crm.USERID, nameof(InvoiceApplyForCustomer_UpdService), @"請款單（廠商）", @"InvoiceApplyForCustomerToAudit（請款單（廠商）提交簽核）", @"", @"", @"");
            }
            finally
            {
                if (null != sMsg)
        ...
        /// <returns></returns>
        public ResponseMessage InvoiceApplyForCustomerAudit(RequestMessage i_crm)
        {
            ResponseMessage rm = null;
            string sMsg = null;
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                        ...
                        var sTitle_Self = @"";
                        var sTitle_Handle = oUser_Self.MemberName + @"審批了" + oAskTheDummy.MemberName + @"的請款單（廠商）「" + oEip.KeyNote + @"」簽呈編號：" + oEip.SignedNumber;
                        var sTitle_Next = oAskTheDummy.MemberName + @"的請款單（廠商）申請「" + oEip.KeyNote + @"」簽呈編號：" + oEip.SignedNumber;
                        var sTitle_Notice = @"";
                        var sStatus = @"B";

                        if (sAction == @"Signed")
                        {
                            sTitle_Self = oUser_Self.MemberName + @"審批了您的請款單（廠商）「" + oEip.KeyNote + @"」簽呈編號：" + oEip.SignedNumber;
                            sTitle_Notice = oAskTheDummy.MemberName + @"的請款單（廠商）申請「" + oEip.KeyNote + @"」簽呈編號：" + oEip.SignedNumber + @"，請點擊查看...";
                        ...
                        {
                            sTitle_Self = oUser_Self.MemberName + @"签辦了您的請款單（廠商）申請「" + oEip.KeyNote + @"」簽呈編號：" + oEip.SignedNumber;
                            ...
                            foreach (string flow in saNextSignedWays)
                            {
                                if (sGoNext == @"Y")
                                {
                                    if (flow == @"flow4")//添加通知和提醒給下個流程所有要通知的人
                                    {
                                        ...
                                        {
                                            //添加提醒消息
                                            var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle_Notice, user, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, WebAppGlobalConstWord.BELL);
                                    ...
                                    else if (flow == @"flow5")//添加通知和提醒給經辦人
                                    {
                                        if (sHandlePerson != @"")
                                        {
                                            ...
                                            {
                                                //添加代辦
                                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, oEip.Guid, sHandlePerson, sTitle_Handle, @"InvoiceApplyForCustomer_View", @"?Action=Upd&Guid=" + oEip.Guid, @"E");
                                                listTask.Add(oTaskAdd);
                                                //添加提醒消息
                                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle_Handle, sHandlePerson, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, WebAppGlobalConstWord.BELL);
                                                listTips.Add(oTipsAdd);
                                    ...
                                    else
                                    {
                                        foreach (string user in saNextUsers)//添加通知和提醒給下一個審核的人
                                        {
                                            ...
                                            {
                                                //添加代辦
                                                var oTaskAdd = SYS.Task_QryService.TaskAdd(i_crm, oEip.Guid, user, sTitle_Next, @"InvoiceApplyForCustomer_View", @"?Action=Upd&Guid=" + oEip.Guid, @"G");
                                                listTask.Add(oTaskAdd);
                                                //添加提醒消息
                                                var oTipsAdd = SYS.Task_QryService.TipsAdd(i_crm, sTitle_Next, user, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, WebAppGlobalConstWord.BELL);
                                                listTips.Add(oTipsAdd);
                                                listToTips.Add(user);
                                            }
                                        }
                        ...
                        //添加提醒消息(給請假的人)
                        var oTips_Applicant = SYS.Task_QryService.TipsAdd(i_crm, sTitle_Self, oEip.Applicant, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, TipsType);
        ...
        #region 請款單（廠商）簽核前面所有人
        /// <summary>
        /// 請款單（廠商）簽核前面所有人
        /// </summary>
        /// <param name="i_crm">todo: describe i_crm parameter on InvoiceApplyForCustomerAudit</param>
        /// <returns></returns>
        public ResponseMessage InvoiceApplyForCustomerAuditAll(RequestMessage i_crm)
        {
            ...
            try
            {
                rm = SugarBase.ExecTran(db =>
                {
                    do
                    {
                            ...
                            case true:
                                {
                                    ...
                                    var oTask = SYS.Task_QryService.TaskAdd(i_crm, oEip.Guid, oEip.Handle_Person, NotificationInfo.Item1, @"InvoiceApplyForCustomer_View", @"?Action=Upd&Guid=" + oEip.Guid, @"E");
                                    InsertTasks.Add(oTask);//經辦人員，新增代辦事項
                                    var oTipsToHandlePerson = SYS.Task_QryService.TipsAdd(i_crm, NotificationInfo.Item1, oEip.Handle_Person, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, TipsType);
                                    ...
                                    {
                                        var _Tip = SYS.Task_QryService.TipsAdd(i_crm, NotificationInfo.Item1 + @"，請點擊查看...", Name, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, TipsType);
                        ...
                        var oTipsToApplicant = SYS.Task_QryService.TipsAdd(i_crm, NotificationInfo.Item1 + NotificationInfo.Item2, oEip.Applicant, @"InvoiceApplyForCustomer_View" + @"|?Action=Upd&Guid=" + oEip.Guid, TipsType);
